...
`macports`, `homebrew`, and `spack`. The automatic dependency discovery described below also
supports `macports` and the `debian`/`ubuntu` package manager, via `pkgdb: macports` and
`pkgdb: dpkg` respectively. Please keep in mind that most enterprise systems provide
...
typos, please file an [issue](https://github.com/aivazis/qed/issues) at the `qed`
...
Again, if anything goes wrong, please [reach out](https://github.com/aivazis/qed/issues),
