...
// collapse the folder at {uri} of the archive at {archive}
export const useCollapseFolder = () => {
    // build the mutation request
    const [request, isInFlight] = useMutation(useCollapseFolderMutation)
    // build the handler
...
export const useCollapseFolderMutation = graphql`
    mutation useCollapseFolderMutation($input: CollapseFolderInput!) {
        collapseFolder(input: $input) {
            archive {
                id
                expanded
                pending
