...
Let's go back to the folder with the NISAR data samples and load a few more. The discussion that
follows assumes that you have loaded the RSLC, GUNW, and GCOV products, and given them simple
nicknames that correspond to their product type. Clicking on the <Badge><Data/></Badge> icon will
bring up a display that should look something like this:
...
introduces the notion of **selectors**, i.e. choices with a discrete set of values that become part
