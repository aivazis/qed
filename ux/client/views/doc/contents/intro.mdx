...
If you are reading this from within `qed`, you should be able to skip this section, as it deals with
the various installation options.

If you are lucky enough to work on a machine that is managed professionally, `qed` may already be
installed. Please contact the system administrators for instructions about how to access it, and skip
to the next section.
...
The build system needs to know wher to find headers and libraries for the `qed` dependencies. The
...
of the `python` version that is used by `mm` to locate the various locations where the interpreter
and its packages deposit fiels we need.
...
``` bash
~/dv> alias mm='python3 ${HOME}/dv/mm/mm.py'
```
...
show a few details about the build. This should generate a few lines of output similar to the
following:
...
builder directory layout:
  staging layout:
           tmp = /Users/mga/tmp/builds/mm/gcc/opt-shared-darwin-arm64/
  install layout:
...
typos, please file an issue at the `qed` [repository](https://github.com/aivazis/qed/issues),
...
Don't worry if you don't have twenty cores on your machine. Most modern machine will be able to
...
If you are installing in a `conda` environment, `qed` is already on your path so there are no
additional steps to take to makeit accessible. If not, you will have to tweak your environment
variables.
