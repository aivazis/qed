...
class qed::nisar::real::MaskedCovariance {
    // types
public:
    // me
    using self_type = MaskedCovariance<sourceT, maskT>;
...
public:
    // destructor
    ~MaskedCovariance() = default;

    // constructors
    MaskedCovariance(const MaskedCovariance &) = default;
    MaskedCovariance(MaskedCovariance &&) = default;
    MaskedCovariance & operator=(const MaskedCovariance &) = default;
    MaskedCovariance & operator=(MaskedCovariance &&) = default;
};


// the definitions
#include "MaskedCovariance.icc"
